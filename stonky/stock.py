...
class Stock:
    ...
    @property
    def delta_percent_str(self) -> str:
    ...
    def volume_str(self) -> str:
        if self.volume >= 1_000_000_000:
            s = f"{self.volume/1_000_00_000:.1f}"
            s = s.rstrip(".0") + "B"
        elif self.volume >= 1_000_000:
            s = f"{self.volume/1_000_00:.1f}"
            s = s.rstrip(".0") + "M"
        elif self.volume >= 1_000:
            s = f"{self.volume/1_000:.1f}"
            s = s.rstrip(".0") + "K"
        else:
            s = f"{self.volume:.2f}"
        return s
    ...
    def ticker_tape(self) -> str:
        ...
        s += symbol
        s += f"{self.delta_amount:+,.2f} {self.delta_percent*100:+.2f}%".rjust(14)
        return s
